...
- `/상점알림 설정` — 계정별로 매일 지정한 한국 시간에 상점 계정을 DM으로 알림
- `/상점알림 해제` — 특정 계정 또는 전체 상점 알림 중지
- `/상점즐겨찾기` — 관심 스킨을 추가·확인·삭제하고, 추가 즉시 또는 상점 갱신·예약 알림 시 등장 계정과 스킨을 DM으로 알림
- `/소식알림` — 관리자 전용. 한국 공식 공지와 한국어 게임 업데이트를 기존 채널 또는 새 전용 채널에 매일 자동 알림
- `/대회` — Riot 공식 한국어 데이터로 경기 일정, 결과, 순위표와 브래킷 결과를 조회
...
기존 Supabase 프로젝트에는 `sql/valorant_news.sql`을 추가로 실행해야 공식 소식 알림을 사용할 수 있습니다.
